...
class ModelEvaluator_pred_unseen:
    def __init__(self, config_file=None):
        self.config = configparser.ConfigParser()
        self.config.read(config_file, encoding='utf-8') #confing.ini에 주석이라도 한글 있을시 encoding 필요
        #self.X = np.array(pd.read_csv(self.config['PREDICTION_UNSEEN']['UNSEEN_X']))
        self.X = pd.read_csv(self.config['PREDICTION_UNSEEN']['UNSEEN_X'], encoding='CP949')
        
        unseen_y_path = self.config['PREDICTION_UNSEEN']['UNSEEN_Y']
        if unseen_y_path == None:
            self.y = None 
        else:
            self.y = np.array(pd.read_csv(unseen_y_path)).flatten()
            
        self.task = self.config['PREDICTION_UNSEEN']['TASK'] 
        self.model = self.config['PREDICTION_UNSEEN']['MODEL'] 
        self.scaler = self.config['PREDICTION_UNSEEN']['SCALER'] 
    ...
    def prediction_part(self, task_name, model_result, save_path = None): # 평가결과 기준은 회귀 모델 : R2, 분류 모델 : F1 
        if task_name == 'regression':
            ...
            if self.y == str(None):
                ...
                for index, (model_name,prediction_value) in enumerate(zip(results_df_regression['Model'],results_df_regression['Predictions'])):
    ...
    def select_model(self):
        ...
        # 결과를 저장할 폴더 생성
        if task == 'regression':
            if not os.path.exists('./Prediction/Regression_unseen'):
                os.makedirs('./Prediction/Regression_unseen')

            result_dir = './Prediction/Regression_unseen'
        else:
            if not os.path.exists('./Classification_unseen'):
                os.makedirs('./Classification_unseen')        
            result_dir = './Classification_unseen'    
        ...
        for target_model in models:
            ...
            # 새로운 데이터 변환 
            scaled_unseen_x = loaded_scaler.named_steps['preprocessor'].transform(self.X).toarray()
            print('scaled_unseen_x ==?>',scaled_unseen_x)
            
            predictions = target_model.predict(scaled_unseen_x)
            ...
            if task == 'classification':
                ...
                if self.y == str(None):
                    print('실제 Y 값 부재로 평가 결과 없음.')
                    mean_score = {'R-squared': None, 'MAE': None, 'MAPE': None, 'RMSE': None}
...
def main():
    ...
    first_model = ModelEvaluator_pred_unseen(config_file=config_file)
